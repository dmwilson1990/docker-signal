...
Signal is a cross-platform messaging app. Privacy isn’t an optional mode – it’s just the way that Signal works. Every message, every call, every time.

---

## Table of Content

   * [Docker container for Signal](#docker-container-for-signal)
...
Launch the Signal docker container with the following command:
```
docker run -d \
    --name=signal \
    -p 5800:5800 \
    -v /docker/appdata/signal:/config:rw \
    dmwilson1990/docker-signal
```

Where:
  - `/docker/appdata/signal`: This is where the application stores its configuration, log and any files needing persistency.
...
services:
  signal:
    image: dmwilson1990/docker-signal
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/signal:/config:rw"
```
...
```
docker pull dmwilson1990/docker-signal
```
...
container image.
  ...
  3.  In the search bar, type the name of the container (`dmwilson1990/docker-signal`).
...
[create a new issue]: https://github.com/dmwilson1990/docker-signal/issues
